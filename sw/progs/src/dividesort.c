...
void merge(int *a1, int *a2, int *dest, int len){		//merge any two sorted arrays and write it to destination
	int *p1=a1;
	int *p2=a2;
	int *p3=dest;
...
int partition(int* p, int start, int end) {
    ...
    int j, tmp, i = start - 1;
    for (j = start; j < end; j++) {
        if (*(p+j)< x) {				//quicksort algorithm that will be running on the slave cores
            i++;
            swap((p+i),(p+j));
        }
    }
...
int main(int a) {
 ...
 int size_each=16;
    ...
    if (cpuid == 0) {	
    	...
    	while(number>1){					//iteratively merging the sorted arrays and overwriting it. Memory space is just 2N where N is the number of elements we sort.
    		count=0;
